public struct SQLDecoder: Decoder {
    ...
    public func singleValueContainer() throws -> any SingleValueDecodingContainer {
        guard row.values.count == 1 else {
            throw DecodingError.dataCorrupted(.init(codingPath: [], debugDescription: "Single value container for SQL row deserialization expects to be ran on result set of size 1, not \(row.values.count)"))
        }
...
fileprivate struct RowDecodingContainer<Key: CodingKey>: KeyedDecodingContainerProtocol {
...
fileprivate struct ValueDecodingContainer: SingleValueDecodingContainer {
